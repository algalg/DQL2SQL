...
type
  ...
  protected
    ...
    function GetAsString: string; virtual;
    function GetResultSQL: string; virtual;
  ...
  public
    ...
    property TableList: string read FTableList;
    property AsString: string read GetAsString;
    property ResultSQL: string read GetResultSQL;
...
begin
  ...
  begin
    DiscardParse;
    OwnerStatement.CurrentSQL.Locate(LastItem);
    OwnerStatement.CurrentSQL.Next;
    Clear;
    InitializeParse;
    FParseStarted := False;
  end;
...
begin
  ...
  begin
    if FTableList = '' then
      FTableList := ATable.AsString
    else
      FTableList := FTableList + ',' + ATable.AsString;
  end;
  if OwnerStm <> nil then
...
begin
  if (UpperCase(OwnerParser.TokenString) = 'SELECT') and (OwnerParser.TokenType = stSymbol) then
    FSelectCount := FSelectCount + 1;

  if OwnerParser.TokenString <> 'skip' then
  begin
...
function TgaCustomSQLStatement.GetResultSQL: string;
var
  s_TempSQL : string;
begin
  s_TempSQL := Trim(CurrentSQL.AsString);
  // If the SQL statement does not have a ';' then add it as it is need to parse the statement properly;
  if Copy(s_TempSQL, length(s_TempSQL) , 1) = ';' then
    s_TempSQL := Copy(s_TempSQL, 1, length(s_TempSQL) - 1);
